...
| ------------ | ----------- | ----------- |
| gpu | 4.5 GB | main example - downloads, builds and installs gpu kernel modules (required)|
| cuda | 21.1 GB | example container with CUDA toolkit installed (optional)|
| app | 8.3 GB | example of app container installing PyTorch (optional) |
| nv-pytorch | 22.6 GB | example of using an Nvidia base image for PyTorch  (optional)|

## How it works
### gpu container
This is the main container in this example and the only one you need to obtain GPU access from within your container or for any other containers in the application. It downloads the kernel source files for our exact OS version and uses them, along with the driver file downloaded from Nvidia to build the required Nvidia kernel modules. Finally, the `entry.sh` file unloads the current Nouveau driver if it's running and loads the Nvidia modules. It also loads firmware onto the GPU from this script if your device requires it.
...
Before using this example, you'll need to make sure that you've set the variables at the top of the gpu and cuda Dockerfiles:
- `OPEN_DRIVER` set to `1` to use the open source driver, or `0` to use the NVIDIA proprietary driver. See notes below for more details.
...
- If you are trying to use this example with balenaOS < 3.0 change any occurrences of `kernel_modules_headers` (such as in the gpu Dockerfile) to `kernel-source`.
- There are two types of drivers available for NVIDIA GPUs: A proprietary one and an open source one. Per NVIDIA, "new GPUs are likely to be better supported by the open driver, while the proprietary driver is better for older cards." This example uses the proprietary driver - see the variable setting above to change to the open driver. Check dmesg for errors that may help you decide which one to use, for example: `NVRM: installed in this system requires use of the NVIDIA open kernel modules.`
- The `entry.sh` file uses the balena extra-firmware feature to make the device firmware available for loading. Make sure to update the path name in the source file path to match the NVIDIA driver version used in your Dockerfile.
...
| 21%   44C    P0              1W /  175W |       0MiB /   8192MiB |      0%      Default |
|                                         |                        |                  N/A |
+-----------------------------------------+------------------------+----------------------+
...
The variables are: `OPEN_DRIVER`, `VERSION`, `BALENA_MACHINE_NAME`, `YOCTO_VERSION` and `NVIDIA_DRIVER_VERSION`
...
services:
  gpu:
    ...
    labels:
      ...
      io.balena.features.requires.sw.linux: '6.12.47'
    volumes:
...
- The error `Failed to initialize NVML: Driver/library version mismatch` usually means you need to reboot the device so a change can take effect. If that does not clear the error, try running `nvidia-smi` in the container to get the "NVML library version" and make sure it matches **exactly** the driver version in the gpu container.
...
Starting in balenaOS 3.0 the `kernel-source` kernel headers artifact was renamed to `kernel-modules-headers`. If you are trying to use this example with balenaOS < 3.0 (please avoid though!) change any occurences of `kernel_modules_headers` (such as in line 30 of the gpu Dockerfile) to `kernel-source`.
